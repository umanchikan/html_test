...
def janken_game(player_hand, enemy_hand)
  rock_paper_scissors = {0 =>"グー", 1=>"チョキ", 2=>"パー"}
  game_rezult = {0 =>"あいこ", 1 =>"負け", 2 =>"勝ち"}
  
  rezult = (player_hand - enemy_hand + 3) % 3
    if rezult == 1 || rezult == 2 #勝ちか負け
      puts "相手の手は#{rock_paper_scissors[enemy_hand]}です。あなたの#{game_rezult[rezult]}です。"
      return
    else
      puts "相手の手は#{rock_paper_scissors[enemy_hand]}です。あなたの#{game_rezult[rezult]}です。"
        player_hand = Player.new
        ...
        #janken = Janken.new 削除
        #janken.janken_game(@player_hand, @enemy_hand) 削除
    end
end
end
janken = Janken.new
